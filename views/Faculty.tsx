...
const ToggleSwitch: React.FC<{ checked: boolean; onChange: () => void; disabled?: boolean }> = ({ checked, onChange, disabled }) => (
   <button
      ...
      onClick={onChange}
      disabled={disabled}
      className={`w-14 h-7 rounded-full p-1 transition-colors duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ${checked ? 'bg-green-500' : 'bg-slate-200'
         } ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
   >
      <div
         className={`bg-white w-5 h-5 rounded-full shadow-md transform transition-transform duration-300 ease-in-out flex items-center justify-center ${checked ? 'translate-x-7' : 'translate-x-0'
            }`}
      >
...
export const FacultyDashboard: React.FC<FacultyProps> = ({ user: propUser }) => {
   ...
   useEffect(() => {
      ...
      } else if (propUser && propUser.uid && user.uid !== propUser.uid) {
         setUser(propUser);
      }
   }, [propUser, user]);
